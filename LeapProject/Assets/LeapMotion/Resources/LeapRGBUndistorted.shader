  ...
  SubShader {
    ...
    Pass {
      ...
      float4 colorCorrect(float texImageX, float texImageY) {
        ...
        l_FragColor.r = rfudge*output_lf_fudge.r + (1-rfudge)*output_lf.r;
        l_FragColor.g = gfudge*output_lf_fudge.g + (1-gfudge)*output_lf.g;
        l_FragColor.b = bfudge*output_lf_fudge.b + (1-bfudge)*output_lf.b;

        l_FragColor.r *= rscale;
        l_FragColor.g *= gscale;
        l_FragColor.b *= bscale;
       
        //float avgrgb = 0.33333*(input_lf.r + input_lf.g + input_lf.b) - 0.9*input_lf.a;
        //float threshold = min(1, avgrgb*100);
        //threshold *= threshold;
        //l_FragColor.rgb = output_lf_gray.rgb*(1 - threshold) + l_FragColor.rgb*(threshold);
       
        float gamma = 1.0/_GammaCorrection;
        //l_FragColor.rgb = show_ir > 0.5 ? float3(pow(ir_out, gamma)): pow(l_FragColor.rgb, float3(gamma));
        
        l_FragColor = float4(pow(l_FragColor.rgb, float(gamma)), 1.0);
        return l_FragColor;
      }
